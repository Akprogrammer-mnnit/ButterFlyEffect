...
async function getGitOriginUrl(workspacePath) {
    try {
...
async function getChangedLines(filePath, workspacePath) {
    try {
...
class ImpactCodeLensProvider {
    ...
    async provideCodeLenses(document) {
        ...
        if (symbols) {
            const findTargetSymbols = (syms) => {
                for (const symbol of syms) {
                    const isValidKind = symbol.kind === vscode.SymbolKind.Function ||
...
function activate(context) {
    /* eslint-disable */ console.log(...oo_oo(`4069791241_153_1_153_57_4`, 'Butterfly Effect extension is now active!'));
    ...
    const analyzeCommand = vscode.commands.registerCommand('butterfly.analyzeFunction', async (functionId, functionCode) => {
        vscode.window.withProgress({
            ...
            try {
                ...
                const response = await axios_1.default.post(`${local}/api/impact/analyze`, {
...
class ImpactReportPanel {
    ...
    static createOrShow(reportData, functionId) {
        ...
        const panel = vscode.window.createWebviewPanel('impactReport', '🦋 Blast Radius Report', column || vscode.ViewColumn.Beside, { enableScripts: true });
    ...
    _getHtmlForWebview(data, functionId) {
        ...
        if (ai.risk_level === 'MEDIUM')
            riskColor = "var(--vscode-testing-iconQueued)";
        if (ai.risk_level === 'HIGH')
            riskColor = "var(--vscode-testing-iconFailed)";
        if (ai.risk_level === 'CRITICAL')
            riskColor = "var(--vscode-errorForeground)";
        let directIssuesHtml = '';
        if (ai.direct_issues && ai.direct_issues.length > 0) {
            directIssuesHtml = `
				<h2>🛑 Errors in Changed Code</h2>
				<ul class="bug-list">
					${ai.direct_issues.map((issue) => `
