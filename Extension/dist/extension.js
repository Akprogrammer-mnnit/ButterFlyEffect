...
var FormData$1 = __webpack_require__(4);
var crypto = __webpack_require__(14);
var url = __webpack_require__(12);
var http = __webpack_require__(10);
var https = __webpack_require__(11);
var http2 = __webpack_require__(65);
var util = __webpack_require__(6);
var followRedirects = __webpack_require__(66);
...
function getChangedLines(filePath, workspacePath) {
    try {
...
function activate(context) {
    /* eslint-disable */ console.log(...oo_oo(`1793928540_82_1_82_57_4`, 'Butterfly Effect extension is now active!'));
    const lensProvider = new ImpactCodeLensProvider();
    vscode.languages.registerCodeLensProvider({ scheme: 'file' }, lensProvider);
    const analyzeCommand = vscode.commands.registerCommand('butterfly.analyzeFunction', async (functionId, functionCode) => {
        vscode.window.withProgress({
            ...
            try {
                /* eslint-disable */ console.log(...oo_oo(`1793928540_99_5_99_57_4`, `Analyzing impact for ${functionId}...`));
                const response = await axios_1.default.post('http://localhost:5555/api/impact/analyze', {
                ...
                /* eslint-disable */ console.error(...oo_tx(`1793928540_117_5_117_25_11`, error));
            }
        });
    });
...
class ImpactReportPanel {
    ...
    _panel;
    _disposables = [];
    static createOrShow(reportData, functionId) {
        ...
        const panel = vscode.window.createWebviewPanel('impactReport', '🦋 Blast Radius Report', column || vscode.ViewColumn.Beside, // Opens in a split pane next to their code!
        { enableScripts: true });
        ImpactReportPanel.currentPanel = new ImpactReportPanel(panel, reportData, functionId);
    }
    constructor(panel, reportData, functionId) {
        this._panel = panel;
        this.update(reportData, functionId);
        this._panel.onDidDispose(() => this.dispose(), null, this._disposables);
    }
    update(reportData, functionId) {
    ...
    _getHtmlForWebview(data, functionId) {
        const ai = data.aiReport;
        // Color code the risk level
        let riskColor = "var(--vscode-testing-iconPassed)"; // Green
        if (ai.risk_level === 'MEDIUM')
            riskColor = "var(--vscode-testing-iconQueued)"; // Yellow
        if (ai.risk_level === 'HIGH')
            riskColor = "var(--vscode-testing-iconFailed)"; // Red
        if (ai.risk_level === 'CRITICAL')
            riskColor = "var(--vscode-errorForeground)"; // Bright Red
        // Generate HTML for the Impact Breakdown
        const breakdownHtml = ai.impact_breakdown.map((item) => `
			<div class="card">
        ...
        const bugsHtml = ai.potential_bugs.map((bug) => `
